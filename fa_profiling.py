...
def jax_attention(
    ...
    window = (
        (-1, -1) if (window_size is None or window_size == -1) else (window_size, 0)
    )
    return _jax_attention(
        q, k, v, segment_ids_q, segment_ids_kv, sm_scale, causal, window
    )
...
def te_fa_forward(
    ...
    window_size = (
        None if (window_size is None or window_size == -1) else (window_size, -1)
    )
    attn_out = te.fused_attn(
        qkv=(q, k, v),
        bias=None,
        sequence_descriptor=te.SequenceDescriptor.from_segment_ids_and_pos(
            (segment_ids_q + 1, segment_ids_kv + 1), None
        ),
        seed=None,
        attn_bias_type=te.AttnBiasType.NO_BIAS,
        attn_mask_type=(
            te.AttnMaskType.PADDING_CAUSAL_MASK
            if causal
            else te.AttnMaskType.PADDING_MASK
        ),
